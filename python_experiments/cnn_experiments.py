...
from cnn_model_lookback import Transition, ReplayMemory, DQN_Lookback
...
# This file should be largely similar to the lstm_experiments.py file with some
# minor changes because it was built upon to write lstm_experiments.py
...
policy_net = DQN_Lookback(screen_height, screen_width, n_actions, lookback=LOOKBACK).to(device)
target_net = DQN_Lookback(screen_height, screen_width, n_actions, lookback=LOOKBACK).to(device)
...
def select_action(state, lstm_inputs, inference=False):
    ...
    if sample > eps_threshold or inference:
        with torch.no_grad():
            ...
            # (h_out, c_out) are just for compatibility with the LSTM functions
            # (lstm_inputs) as well
...
# NOTE: This is an older version of this function as it takes in "lstm_inputs"
# though it should be fine even if None is passed in
def optimize_model(lstm_inputs):
    ...
    (action_logits, action_values, (h_out, c_out)) = policy_net(state_batch, lstm_inputs)
    ...
    t_action_logits, _, _ = target_net(non_final_next_states, lstm_inputs)
...
# the following MEMORY variables will be used to construct the state with lookback
VISION_MEMORY = torch.zeros((1, screen_height, screen_height, channels*LOOKBACK), device=device)
SCENT_MEMORY = torch.zeros((1, env._agent.scent().shape[-1] * LOOKBACK), device=device)
...
for i_step in range(NUM_STEPS):
    ...
    curr_vision = torch.tensor(env._agent.vision(), device=device).view((1, screen_height, screen_width, channels))
    curr_scent = torch.tensor(env._agent.scent(), device=device).view((1, -1))

    VISION_MEMORY = torch.cat((curr_vision, VISION_MEMORY), 3)[:, :, :, :-3]
    # hardcoding scent dimensionality
    SCENT_MEMORY = torch.cat((curr_scent, SCENT_MEMORY), 1)[:, :-3]
    ...
    # Observe new state
    obs_vision = torch.tensor(observation['vision'], device=device).view((1, screen_height, screen_width, channels))
    obs_scent = torch.tensor(observation['scent'], device=device).view((1, -1))

    next_state = (
        torch.cat((obs_vision, VISION_MEMORY), 3)[:, :, :, :-3],
        torch.cat((obs_scent, SCENT_MEMORY), 1)[:, :-3],
        observation['moved']
    )
    ...
    optim_out = optimize_model(LSTM_INPUTS)
...
torch.save(policy_net.state_dict(), './cnn_jbw')
